...
frappe.ui.Page = Class.extend({
	...
	add_dropdown_item: function add_dropdown_item(label, click, standard, parent) {
		var is_already_added = function is_already_added() {
			var found_lists = $(parent).find('li > a.grey-link:contains(' + label + ')');
...
frappe.views.CommunicationComposer = Class.extend({
	...
	setup_earlier_reply: function setup_earlier_reply() {
		...
		if (last_email) {
			...
			content = '<div><br></div>' + reply + "<br><!-- original-reply --><br>" + '<blockquote>' + '<p>' + __("On {0}, {1} wrote:", [frappe.datetime.global_date_format(last_email.communication_date), last_email.sender]) + '</p>' + last_email_content + '<blockquote>';
		} else {
